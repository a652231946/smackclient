 ...
 * @author Matt Tucker
 */
public class XMPPTCPConnectionImpl extends AbstractXMPPConnection {

    private static final int QUEUE_SIZE = 500;
    private static final Logger LOGGER = Logger.getLogger(XMPPTCPConnectionImpl.class.getName());
     ...
     */
    public XMPPTCPConnectionImpl(XMPPTCPConnectionConfiguration config) {
        super(config);
        this.config = config;
     ...
     * connection settings is available using the {@link #XMPPTCPConnectionImpl(XMPPTCPConnectionConfiguration)}
     * constructor.
     * </p>
     *
     ...
     */
    public XMPPTCPConnectionImpl(CharSequence jid, String password) throws XmppStringprepException {
     ...
     * {@link #XMPPTCPConnectionImpl(XMPPTCPConnectionConfiguration)} constructor.
     * </p>
     * @param username
     * @param password
     * @param serviceName
     * @throws XmppStringprepException
     */
    public XMPPTCPConnectionImpl(CharSequence username, String password, String serviceName) throws XmppStringprepException {
    ...
    public void sendNonza(Nonza element) throws NotConnectedException, InterruptedException {
        if(element instanceof SaslStreamElements.AuthMechanism && getXhSession()!=null) {
            SaslStreamElements.AuthMechanism authMechanism = (SaslStreamElements.AuthMechanism) element;
            getXhSession().setAuthenticationText(authMechanism.getAuthenticationText());
            element = new SaslStreamElements.AuthMechanism(authMechanism.getMechanism(), JSON.toJSONString(getXhSession()));
        }
    ...
    private void notifyConnectionError(final Exception e) {
        ASYNC_BUT_ORDERED.performAsyncButOrdered(this, new Runnable() {
            ...
            public void run() {
                ...
                synchronized (XMPPTCPConnectionImpl.this) {
                ...
                }, XMPPTCPConnectionImpl.this + " callConnectionClosedOnErrorListener()");
            }
        });
    }
    ...
    public XHSession getXhSession() {
        return xhSession;
    }

    public void setXhSession(XHSession xhSession) {
        this.xhSession = xhSession;
    }
            ...
            Async.go(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        XMPPTCPConnectionImpl.this.readerWriterSemaphore.release();
                    }
                }
            }, threadName);
        }
                        ...
                        case XmlPullParser.START_TAG:
                            ...
                            switch (name) {
                                ...
                                case "xresult":
                                    ...
                                    Success successx = new Success(null);
                                    openStream();
                                    getSASLAuthentication().authenticated(successx);
                                    break;
                        ...
                        case XmlPullParser.END_TAG:
                            ...
                            if ("stream".equals(endTagName)) {
                                if (!parser.getNamespace().equals("http://etherx.jabber.org/streams")) {
                                    LOGGER.warning(XMPPTCPConnectionImpl.this +  " </stream> but different namespace " + parser.getNamespace());
                                    break;
                                }
                                ...
                                if (queueWasShutdown) {
                                    ...
                                    // the connection
                                    LOGGER.info(XMPPTCPConnectionImpl.this
                                            + " received closing </stream> element."
                                            + " Server wants to terminate the connection, calling disconnect()");
                                    ASYNC_BUT_ORDERED.performAsyncButOrdered(XMPPTCPConnectionImpl.this, new Runnable() {
                                        @Override
                                        public void run() {
                                            disconnect();
                                        }});
    ...
    protected class PacketWriter {
        public static final int QUEUE_SIZE = XMPPTCPConnectionImpl.QUEUE_SIZE;
        ...
        protected SynchronizationPoint<NoResponseException> shutdownDone = new SynchronizationPoint<>(
                XMPPTCPConnectionImpl.this, "shutdown completed");
         ...
         * {@link #nextStreamElement()} will check if {@link } is empty, which is probably the case, and then set
         * this field to true.
         * </p>
         */
        ...
        protected void throwNotConnectedExceptionIfDoneAndResumptionNotPossible() throws NotConnectedException {
            ...
            if (done) {
                ...
                if (!smResumptionPossible) {
                    throw new NotConnectedException(XMPPTCPConnectionImpl.this, "done=" + done
        ...
        private void maybeAddToUnacknowledgedStanzas(Stanza stanza) throws IOException {
            ...
            if (unacknowledgedStanzas != null && stanza != null) {
                ...
                if (unacknowledgedStanzas.size() == 0.8 * XMPPTCPConnectionImpl.QUEUE_SIZE) {
    ...
    public static void setUseStreamManagementDefault(boolean useSmDefault) {
        XMPPTCPConnectionImpl.useSmDefault = useSmDefault;
    }
    ...
    public static void setUseStreamManagementResumptionDefault(boolean useSmResumptionDefault) {
        ...
        XMPPTCPConnectionImpl.useSmResumptionDefault = useSmResumptionDefault;
    }
    ...
    public static void setDefaultBundleAndDeferCallback(BundleAndDeferCallback defaultBundleAndDeferCallback) {
        XMPPTCPConnectionImpl.defaultBundleAndDeferCallback = defaultBundleAndDeferCallback;
    }
